...
public class BoardTest {
    ...
    public void testOppositeDir() {
        Board b0 = new Board(BOARD1, BP);
        for (int i = 0; i < 8; i++) {
            assertEquals(b0.oppositeDir(i), (i + 4) % 8);
        }
    }
    ...
    public void testNumPieces() {
        Board b0 = new Board(BOARD1, BP);
        assertEquals(b0.numberOfPieces(ALL_SQUARES[9], 3), 2);
        assertEquals(b0.numberOfPieces(ALL_SQUARES[9], 1), 1);
        assertEquals(b0.numberOfPieces(ALL_SQUARES[9], 0), 3);
        assertEquals(b0.numberOfPieces(ALL_SQUARES[9], 4), 3);
        assertEquals(b0.numberOfPieces(ALL_SQUARES[9], 6), 3);
    }
